...
class StaticHarmonyGenerator extends HarmonyGenerator {
    constructor(options) {
        ...
        this.baseRoot = getValueOrDefault(options, "baseRoot", 0);
        ...
        this.baseToBaseLikelihood = getValueOrDefault(options, "baseToBaseLikelihood", 0.01);
        this.baseExpandedLikelihood = getValueOrDefault(options, "baseExpandedLikelihood", 1);
        this.baseToNeighbourLikelihood = getValueOrDefault(options, "baseToNeighbourLikelihood", 1);
        this.baseToPassingLikelihood = getValueOrDefault(options, "baseToPassingLikelihood", 1);
        this.baseToAuxiliaryLikelihood = getValueOrDefault(options, "baseToAuxiliaryLikelihood", 1);
        this.auxiliaryToAuxiliaryLikelihood = getValueOrDefault(options, "auxiliaryToAuxiliaryLikelihood", 0.01);
        this.auxiliaryExpandedLikelihood = getValueOrDefault(options, "auxiliaryExpandedLikelihood", 1);
        this.auxiliaryToBaseLikelihood = getValueOrDefault(options, "auxiliaryToBaseLikelihood", 1);
        this.auxiliaryToNeighbourLikelihood = getValueOrDefault(options, "auxiliaryToNeighbourLikelihood", 1);
        this.auxiliaryToPassingLikelihood = getValueOrDefault(options, "auxiliaryToPassingLikelihood", 1);
        this.baseToBaseCost = getValueOrDefault(options, "baseToBaseCost", 0);
        this.baseExpandedCost = getValueOrDefault(options, "baseExpandedCost", 0);
        this.baseToNeighbourCost = getValueOrDefault(options, "baseToNeighbourCost", 0);
        this.baseToPassingCost = getValueOrDefault(options, "baseToPassingCost", 0);
        this.baseToAuxiliaryCost = getValueOrDefault(options, "baseToAuxiliaryCost", 0);
        this.auxiliaryToAuxiliaryCost = getValueOrDefault(options, "auxiliaryToAuxiliaryCost", 0);
        this.auxiliaryExpandedCost = getValueOrDefault(options, "auxiliaryExpandedCost", 0);
        this.auxiliaryToBaseCost = getValueOrDefault(options, "auxiliaryToBaseCost", 0);
        this.auxiliaryToNeighbourCost = getValueOrDefault(options, "auxiliaryToNeighbourCost", 0);
        this.auxiliaryToPassingCost = getValueOrDefault(options, "auxiliaryToPassingCost", 0);
        this.auxiliaryChordRoots = getValueOrDefault(options, "auxiliaryChordRoots", [3, 4, 2, 5]);
        this.auxiliaryChordRootLikelihoods = getValueOrDefault(options, "auxiliaryChordRootLikelihoods", [1, 1, 0.2, 0.2]);
        this.auxiliaryChordRootCosts = getValueOrDefault(options, "auxiliaryChordRootCosts", [0, 0, 0, 0]);
        this.minorPassingChordRoots = getValueOrDefault(options, "minorPassingChordRoots", [0, 1, 2, 3, 4, 5, 6]);
        this.minorPassingChordInversions = getValueOrDefault(options, "minorPassingChordInversions", [[1], [1], [1], [1], [1], [1], [1]]);
        this.majorPassingChordRoots = getValueOrDefault(options, "majorPassingChordRoots", [0, 1, 2, 3, 4, 5, 6]);
        this.majorPassingChordInversions = getValueOrDefault(options, "majorPassingChordInversions", [[1], [1], [1], [1], [1], [1], [1]]);
        this.passingIncrements = getValueOrDefault(options, "passingIncrements", [-2, -1, 1, 2]);
        this.passingIncrementLikelihoods = getValueOrDefault(options, "passingIncrementLikelihoods", [0.25, 1, 1, 0.25]);
        this.passingIncrementCosts = getValueOrDefault(options, "passingIncrementCosts", [0, 0, 0, 0]);
        ...
        this.canEndWithBase = getValueOrDefault(options, "canEndWithBase", true);
        this.canEndWithAuxiliary = getValueOrDefault(options, "canEndWithAuxiliary", false);
        this.possibleAuxiliaryEndRoots = getValueOrDefault(options, "possibleAuxiliaryEndRoots", [3, 4, 2, 5]);
        this.possibleAuxiliaryEndInversions = getValueOrDefault(options, "possibleAuxiliaryEndInversions", [[0], [0], [0], [0]]);
        this.possiblePassingEndRoots = getValueOrDefault(options, "possiblePassingEndRoots", [0]);
        this.possiblePassingEndInversions = getValueOrDefault(options, "possiblePassingEndInversions", [[0, 1]]);
        this.possibleNeighbourEndRoots = getValueOrDefault(options, "possibleNeighbourEndRoots", [0]);
        this.possibleNeighbourEndInversions = getValueOrDefault(options, "possibleNeighbourEndInversions", [[0]]);
        this.baseSeventhLikelihoods = getValueOrDefault(options, "baseSeventhLikelihoods", [[1, 1, 1, 1, 1, 1, 1]]);
        this.baseSeventhCosts = getValueOrDefault(options, "baseSeventhCosts", [[0, 0, 0, 0, 0, 0, 0]]);
        this.baseTriadLikelihoods = getValueOrDefault(options, "baseTriadLikelihoods", [[1, 1, 1, 1, 1, 1, 1]]);
        this.baseTriadCosts = getValueOrDefault(options, "baseTraidCosts", [[0, 0, 0, 0, 0, 0, 0]]);
        this.auxiliarySeventhLikelihoods = getValueOrDefault(options, "auxiliarySeventhLikelihoods", [[1, 1, 1, 1, 1, 1, 1]]);
        this.auxiliaryTriadLikelihoods = getValueOrDefault(options, "auxiliaryTriadLikelihoods", [[1, 1, 1, 1, 1, 1, 1]]);
        this.passingSeventhLikelihoods = getValueOrDefault(options, "passingSeventhLikelihoods", [[1, 1, 1, 1, 1, 1, 1]]);
        this.passingTriadLikelihoods = getValueOrDefault(options, "passingTriadLikelihoods", [[1, 1, 1, 1, 1, 1, 1]]);
        ...
        this.auxiliarySeventhCosts = getValueOrDefault(options, "auxiliarySeventhCosts", [[0, 0, 0, 0, 0, 0, 0]]);
        this.auxiliaryTriadCosts = getValueOrDefault(options, "auxiliaryTriadCosts", [[0, 0, 0, 0, 0, 0, 0]]);
        this.passingSeventhCosts = getValueOrDefault(options, "passingSeventhCosts", [[0, 0, 0, 0, 0, 0, 0]]);
        this.passingTriadCosts = getValueOrDefault(options, "passingTriadCosts", [[0, 0, 0, 0, 0, 0, 0]]);
        ...
        this.startWithAccented64Likelihood = getValueOrDefault(options, "startWithAccented64Likelihood", 110);
        this.startWithoutAccented64Likelihood = getValueOrDefault(options, "startWithoutAccented64Likelihood", 1);
        this.startWithAccented64Cost = getValueOrDefault(options, "startWithAccented64Cost", 10);
        this.startWithoutAccented64Cost = getValueOrDefault(options, "startWithoutAccented64Cost", 0);
    ...
    getStartStateIterator() {
        ...
        this.getChordsStuff(0, state, this.startWithoutAccented64Likelihood, this.startWithoutAccented64Cost, this.baseSeventhLikelihoods, this.baseTriadLikelihoods, this.baseSeventhCosts, this.baseTriadCosts, states, likelihoods, costs);
    ...
    getPassingTowardsTargetStatesAndLikelihoods(node, nextStates, nextLikelihoods, nextCosts) {
        ...
        const passingChordRoots = this.minorPassingChordRoots;
        const passingChordInversions = this.minorPassingChordInversions;
        ...
        for (let i = 0; i < harmonies.length; i++) {
            ...
            state.mode = modes[i];
            this.getChordsStuff(depth, state, likelihoods[i], costs[i], this.passingSeventhLikelihoods, this.passingTriadLikelihoods, this.passingSeventhCosts, this.passingTriadCosts, nextStates, nextLikelihoods, nextCosts);
        }
    }
    getBaseStatesAndLikelihoods(node, nextStates, nextLikelihoods, nextCosts) {
        ...
        if (this.baseToAuxiliaryLikelihood > 0) {
            ...
            for (let i = 0; i < this.auxiliaryChordRoots.length; i++) {
                const auxRoot = this.auxiliaryChordRoots[i];
                const auxLikelihood = this.auxiliaryChordRootLikelihoods[i % this.auxiliaryChordRootLikelihoods.length];
                const auxCost = this.auxiliaryChordRootCosts[i % this.auxiliaryChordRootCosts.length];
                const auxState = this.getAuxiliaryState(auxRoot);
                this.getChordsStuff(depth, auxState, auxLikelihood, auxCost, this.auxiliarySeventhLikelihoods, this.auxiliaryTriadLikelihoods, this.auxiliarySeventhCosts, this.auxiliaryTriadCosts, auxStates, auxLikelihoods, auxCosts);
            }
        ...
        if (this.baseToNeighbourLikelihood > 0 && currentHarmony.chordInversions == 0) {
            ...
            for (let i = 0; i < harmonies.length; i++) {
                ...
                this.getChordsStuff(depth, state, likelihood, this.baseToNeighbourCost, neighbourSeventhLikelihoods, neighbourTriadLikelihoods, neighbourSeventhCosts, neighbourTriadCosts, nextStates, nextLikelihoods, nextCosts);
            }
        }
        if (this.baseToPassingLikelihood > 0) {
            ...
            for (let i = 0; i < harmonies.length; i++) {
                ...
                this.getChordsStuff(depth, state, this.baseToPassingLikelihood * likelihoods[i], this.baseToPassingCost + costs[i], this.passingSeventhLikelihoods, this.passingTriadLikelihoods, this.passingSeventhCosts, this.passingTriadCosts, nextStates, nextLikelihoods, nextCosts);
            }
        }
    }
    getAuxiliaryStatesAndLikelihoods(node, nextStates, nextLikelihoods, nextCosts) {
        ...
        if (this.auxiliaryToAuxiliaryLikelihood > 0) {
            const nextState = new StaticHarmonyState();
