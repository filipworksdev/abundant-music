    ...
    modifyConstantHarmonyElements(elements, module) {
        ...
        if (result.length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                const likelihood = getItemFromArrayWithStartEndItems(1, this.indexLikelihoods, result.length, i, this.startIndexLikelihoods, this.endIndexLikelihoods);
            ...
            function getBadRepeatArray(elements) {
                ...
                for (let i = 0; i < elements.length; i++) {
                    ...
                    if (i < elements.length - 1) {
                        cost = HarmonyGenerator.prototype.calculateBeatStrengthRepetitionCost(elements[i], startBeatStrengths[i], elements[i + 1], startBeatStrengths[i + 1]);
                    }
                    crossesArr[i] = cost > 0;
            ...
            for (const beatsToTest of this.totalBeats) {
                ...
                for (let i = 0; i < this.maxAttempts; i++) {
                    ...
                    const minBeatLength = positionUnitToBeats(this.minElementLength, this.minElementLengthUnit, toShorten.tsNumerator, toShorten.tsDenominator);
